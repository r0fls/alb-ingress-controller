...
func GetInternetFacingConfigMap(namespace, ingressName string) map[string]string {
...
func IngressAllowedExternal(namespace, ingressName string) bool {
	cm := GetInternetFacingConfigMap(namespace, ingressName)
	for ns, ing := range cm {
